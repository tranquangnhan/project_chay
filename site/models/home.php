...
class Model_home extends Model_db{
    ...
    function luudonhangnhe($idDH, $fname,$lname, $email,$phone,$address,$note,$tongtien){            
        if ($idDH==-1){
        $sql = "INSERT INTO donhang SET firstname=?,lastname=? ,email=?,phone=?,address=?,note=?,total=?,ngaydat=Now()";          
        $kq= $this->getLastId($sql,$fname,$lname,$email,$phone,$address,$note,$tongtien);
       ...
       {
        $sql = "UPDATE donhang SET firstname=?,lastname=? ,email=?,phone=?,address=?,note=?,total=?,ngaydat=Now() WHERE id=?";              
         $kq= $this->exec1($sql,$fname,$lname,$email,$phone,$address,$note,$tongtien,$idDH);
      if ($kq == null) return false;
            else return $idDH;
      }
    }
