...
class Model_product extends Model_db{
    ...
    function addNewProduct($name,$name_ge,$slug,$price,$discount,$imgs,$IDCate,$hot,$size,$color,$Description,$Description_ge,$Properties)
    {
        $sql = "INSERT INTO product(name,name_ge,slug,price,discount,image_list,catalog_id,hot,size,color,description,description_ge,properties) VALUE(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        return $this->getLastId($sql,$name,$name_ge,$slug,$price,$discount,$imgs,$IDCate,$hot,$size,$color,$Description,$Description_ge,$Properties);
    }
    ...
    function editProduct($name,$name_ge,$slug,$price,$discount,$imgs,$IDCate,$hot,$size,$color,$Description,$Description_ge,$Properties,$id){
        if($imgs == "")
        {
            $sql = "UPDATE product SET name= ?,name_ge=?,slug=?,price=?,discount=?,catalog_id=?,hot=?,size=?,color=?,description=?,description_ge=?,properties=? WHERE id=?";
            return $this->exec1($sql,$name,$name_ge,$slug,$price,$discount,$IDCate,$hot,$size,$color,$Description,$Description_ge,$Properties,$id);
        }else
        {
            $sql = "UPDATE product SET name= ?,name_ge=?,slug=?,price=?,discount=?,image_list=?,catalog_id=?,hot=?,size=?,color=?,description=?,description_ge=?,properties=? WHERE id=?";
            return $this->exec1($sql,$name,$name_ge,$slug,$price,$discount,$imgs,$IDCate,$hot,$size,$color,$Description,$Description_ge,$Properties,$id);
        }
    }
